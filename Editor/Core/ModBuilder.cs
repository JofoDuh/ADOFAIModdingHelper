	...
	public class ModBuilder
	{
		...
		public async Task<string> Build([CanBeNull] string copyDestination, bool allPlatforms, [CanBeNull] HashSet<BuildTarget> buildTargets)
		{
			try
			{
				...
				var name = "NoName_Null";
				if (modInfo != null) 
					name = !string.IsNullOrWhiteSpace(modInfo.Id) ? modInfo.Id : $"NoName_{modInfo.GetInstanceID()}";
		...
		private void WriteModInfo() => modInfo.WriteToFile(Path.Combine(_buildPath, "Info.json"));
